...
export default function QrModal({ handleCloseScan, data, handleReturnHome }) {
  ...
  const checkData = async () => {
    ...
    for (let i = 1; i <= ticketCount; i++) {
      ...
      if (
        (await nft.getOwner(ticket.idEntrada)).toLowerCase() ===
        account.toLowerCase()
      ) {
  ...
  const checkTicket = async (evento, ticketExist) => {
    if (ticketExist) {
      try {
        await nft.useTicket(ticketScanned.id, evento.idEvento);
        setCanAccess(true);
        setLoading(false);
      } catch (e) {
        console.log(e);
            ...
            <>
              <View style={styles.backgroundWhite}>
                ...
                <View style={styles.ticketContainer}>
                  <Image
                    source={{ uri: ticketScanned.image }}
                    style={[styles.ImageNFT, styles.shadow]}
                  />
                  <Text
                    style={{
                      fontSize: 19,
                ...
                <View style={[styles.buttonsContainer]}>
                  <TouchableOpacity
                    style={[styles.buttonWhite, styles.shadow]}
            ...
            <>
              <View style={styles.backgroundGray}>
                ...
                <View style={styles.modalWhite}>
                  ...
                  >
                    Either you don’t have an NFT for this event, or your ticket
                ...
                <TouchableOpacity
                  style={[styles.buttonGray, styles.shadow]}
                  onPress={() => handleCloseScan()}
                >
